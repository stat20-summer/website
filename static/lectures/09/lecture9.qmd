...
5. Now let's focus again on the first race. Assume that if one horse crosses the finish line, the winning probabilities automatically adjust to be normalized among the remaining horses. What is the probability that Mo Donegal is followed directly by Nest across the finish line (the horse that did end up finishing second in the race)? 
...
Describes a random variable that is the sum of $n$ independent Bernoulli RVs, each with the same success probability $p$.
...
vals <- c(0, 1, 2, 3, 4)
py <- dbinom(vals, size = 4, prob = .5)
df <- data.frame(vals = factor(vals),
                 py = py)
names(df) <- c("x", "P(X = x)")
df %>%
  ggplot(aes(x = x, 
